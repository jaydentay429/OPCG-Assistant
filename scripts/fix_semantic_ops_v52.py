#!/usr/bin/env python3
"""Deterministic semantic fixes v52: high missing-gate / wrong-target / missing-op / gate batch.
...
OUT_IDS = ROOT / "meta" / "logs" / "ops_v52_fixed_ids.txt"
REBUILDS_PATH = ROOT / "meta" / "logs" / "ops_v52_rebuilds.json"
...
def main(argv: list[str] | None = None) -> int:
    ...
    REBUILDS: dict[str, list[dict[str, Any]]] = json.loads(REBUILDS_PATH.read_text())
    ...
    for cid in ordered:
        ...
        if json.dumps(prev.get("abilities"), sort_keys=True, ensure_ascii=False) == json.dumps(
            ...
            # still record for re-review if listed
            if cid not in fixed:
                fixed.append(cid)
            continue
        if args.dry_run:
