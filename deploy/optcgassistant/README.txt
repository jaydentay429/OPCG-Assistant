...
代码目录（推荐，无空格）
  /opt/opcg/app
  = 本仓库「Chinese HK」文件夹内的全部内容（含 app.py、frontend/、deploy/、cards/、meta/ 等）。

一次性准备（在 VPS 上）
  若仅用 root： mkdir -p /opt/opcg/app
  若用 ubuntu：  sudo mkdir -p /opt/opcg && sudo chown ubuntu:ubuntu /opt/opcg
  # 用 git clone + 拷贝，或 scp/rsync 把整个「Chinese HK」同步到 /opt/opcg/app
...
日常更新（在 Mac 本机）
  ...
  rsync -avz --delete \
    ...
    --exclude 'meta/market_prices.json' \
    --exclude 'meta/photo_hash_index.json' --exclude 'meta/photo_clip_index.npz' \
    "/Users/jayden/Documents/OPCG_Project/Chinese HK/" \
...
比赛卡组（ONE PIECE TOP DECKS）
  ...
  cd /opt/opcg/app   # 或 Chinese HK 目录
  ...
  crontab:
    0 6 * * * /opt/opcg/app/deploy/optcgassistant/run_daily_topdecks_sync.sh >> /opt/opcg/logs/daily_topdecks_cron.log 2>&1
...
每日价格同步（VPS cron，12:00 HKT / 04:00 UTC）
  每天刷新「已有现价」的卡（看涨跌）+ 官方新加入的卡；已知无价/miss 先跳过。
  bash /opt/opcg/app/deploy/optcgassistant/run_daily_price_sync.sh
  日志：/opt/opcg/logs/daily_price_latest.log
  crontab:
    0 4 * * * /opt/opcg/app/deploy/optcgassistant/run_daily_price_sync.sh >> /opt/opcg/logs/daily_price_cron.log 2>&1
...
把线上价格拉回本机
  rsync -avz root@165.245.180.246:/opt/opcg/app/meta/market_prices.json \
    "/Users/jayden/Documents/OPCG_Project/Chinese HK/meta/market_prices.json"
