"""EB05-017 Mermaid Cafe Dancers: [On Opp Attack] may rest 1 DON!! + trash this: Fish-Man/Merfolk Leader +4000 battle."""
...
def catalog(cid: str) -> dict:
    ...
    if isinstance(row, dict):
        return row
    return {"card_id": cid, "card_type": "CHARACTER", "cost": 1, "power": 0}
...
def _state(*, leader: str = "OP06-020", don_active: int = 2) -> MatchState:
    p0 = PlayerState(
        ...
        turns_completed=1,
        characters=[CardInst(iid="dancers", card_id="EB05-017", rested=False, summoning_sick=False)],
    )
    p1 = PlayerState(
        seat=1,
        user_id="u1",
...
def test_eb05_017_override_shape():
    reload_effect_library(force=True)
    entry = get_card_entry("EB05-017")
    abs_ = [a for a in entry["abilities"] if a.get("ops")]
    assert abs_, "EB05-017 abilities dropped on normalize"
    ab = next(a for a in get_abilities("EB05-017", "on_opponent_attack"))
    assert ab.get("once") is not True
    ...
    assert ab["ops"][1].get("op") == "trash"
    assert ab["ops"][1].get("target_kind") == "self"
    assert ab["ops"][1].get("as_cost") is True
    assert ab["ops"][2].get("op") == "buff"
    assert ab["ops"][2].get("amount") == 4000
    assert ab["ops"][2].get("target_kind") == "leader"
    assert ab["ops"][2].get("duration") == "battle"


def test_eb05_017_rest_don_trash_self_buffs_leader():
    reload_effect_library(force=True)
    ...
    assert "EB05-017" in st.players[0].trash
    assert not any(c.iid == "dancers" for c in st.players[0].characters)
    after = inst_power(st, 0, "leader", catalog)
    assert after == before + 4000


def test_eb05_017_gated_without_fish_man_leader():
    reload_effect_library(force=True)
...
if __name__ == "__main__":
    test_eb05_017_override_shape()
    test_eb05_017_rest_don_trash_self_buffs_leader()
    test_eb05_017_gated_without_fish_man_leader()
    print("OK")
